...
/**
 * Номер задачи в том виде, в каком его выдаёт beads: буквы, цифры, дефис,
 * точка, подчёркивание. Проверка нужна не для красоты: номер уезжает в
 * аргументы `bd`, и значение вроде `--db=/чужой/путь` заставило бы его
 * читать другую базу. Оболочки в вызове нет, так что подставить команду
 * нельзя, но флаг — можно.
 */
const ISSUE_ID = /^[A-Za-z0-9][A-Za-z0-9._-]*$/

export function isIssueId(value: string): boolean {
  return ISSUE_ID.test(value)
}
